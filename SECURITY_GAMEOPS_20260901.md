...
6. **比賽結算接口**：新增 `public.godot_match_settle`，以比賽 UUID 去重，驗證聯盟與獎勵上限後才更新資源帳本。客戶端全面切換前，仍保留原本離線結算，避免未完成的跨版本同步造成誤扣。

## 仍需在下一個版本接上的部分

- 將比賽、任務、簽約、球探、訓練、預測獎勵的本地加減，逐一改為呼叫 `godot_economy_apply`，成功回應後才更新畫面與本機存檔。
- 將結算畫面改為先呼叫 `godot_match_settle`，成功後再寫入本機 `last_match_gain` 與雲端存檔；離線場次要進入待同步佇列，禁止重複領獎。
