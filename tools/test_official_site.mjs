...
const edge=await browser.newContext();const ep=await edge.newPage();await ep.route('**/rest/v1/app_releases?**',r=>r.fulfill({status:503,body:'unavailable'}));await ep.goto(base+'/',{waitUntil:'networkidle'});assert.ok((await ep.locator('[data-direct-apk]').getAttribute('href')).includes('android-181/'));await edge.close();
