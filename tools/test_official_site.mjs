...
const paths=['/','/news/','/news/build-182.html','/news/android-177.html','/news/test-guide.html','/guide.html','/support.html','/privacy.html','/legal.html','/404.html'];
...
for(const viewport of [{width:1440,height:1000},{width:390,height:844},{width:320,height:568},{width:844,height:390}]) {
  ...
  for(const path of paths){
    ...
    if(path==='/' || (viewport.width===390 && ['/news/build-182.html','/support.html'].includes(path))) await page.screenshot({path:out+'/'+viewport.width+'-'+(path==='/'?'home':path.replaceAll('/','_'))+'.png',fullPage:true});
...
const edge=await browser.newContext();const ep=await edge.newPage();await ep.route('**/rest/v1/app_releases?**',r=>r.fulfill({status:503,body:'unavailable'}));await ep.goto(base+'/',{waitUntil:'networkidle'});assert.ok((await ep.locator('[data-direct-apk]').getAttribute('href')).includes('android-182/'));await edge.close();
const behavior=await browser.newContext({permissions:['clipboard-read','clipboard-write']});const bp=await behavior.newPage();
await bp.goto(base+'/news/build-182.html');await bp.locator('.share-button').click();await bp.waitForFunction(()=>document.querySelector('.share-status').textContent==='連結已複製');
...
const gameHtml=await api.request.get(base+'/game/index.html');assert.equal(gameHtml.status(),200);const gameMarkup=await gameHtml.text();const configuredPackBytes=Number(gameMarkup.match(/"index\.pck":(\d+)/)?.[1]);const packParts=[...gameMarkup.matchAll(/index\.pck\.part-([a-z]{2})\?v=/g)].map(match=>match[1]);let packBytes=0;for(const suffix of packParts){const part=await api.request.head(base+'/game/index.pck.part-'+suffix,{headers:{'Accept-Encoding':'identity'}});assert.equal(part.status(),200,'web pack part '+suffix);packBytes+=Number(part.headers()['content-length']);}assert.ok(gameMarkup.includes('loadSplitMainPack'));assert.ok(configuredPackBytes>0);assert.equal(packBytes,configuredPackBytes,'split web pack size');
