...
"""
	...
	func _regenerate_entitlements_file() -> void:
		# This project uses only a custom URL scheme. Associated-domain
		# entitlements are only required for http(s) universal links, and the
		# upstream exporter expects an .ipa path when generating those.
		var __has_universal_links := false
		for __config in _export_config.deeplinks:
			if __config.scheme.to_lower() in UNIVERSAL_LINK_SCHEMES:
				__has_universal_links = true
				break
		if not __has_universal_links:
			return
		if _export_path:
